...
import { HiHome, HiQrCode, HiDocumentArrowUp, HiDocumentText, HiLockClosed, HiArrowRightOnRectangle, HiUserCircle, HiChartBar, HiCalendar, HiNewspaper, HiUsers, HiBars3, HiXMark } from "react-icons/hi2";
...
export function Sidebar() {
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <div className={`flex h-full flex-col bg-white ${isMobile ? "w-80" : "w-64"}`}>
      {/* Header */}
        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 z-[70] h-screen shadow-2xl md:hidden"
            >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200 bg-white/95 backdrop-blur-lg px-2 pb-safe pt-2 shadow-[0_-8px_30px_rgba(0,0,0,0.12)]">
        <div className="flex justify-around items-center">
          {navItems.slice(0, 5).map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`group relative flex flex-col items-center gap-1.5 p-3 rounded-2xl transition-all duration-300 ${isActive
          ...
          <button
            ...
            className="group relative flex flex-col items-center gap-1.5 p-3 rounded-2xl text-slate-400 hover:text-slate-600 transition-all duration-300"
          >
            <HiBars3 className="relative z-10 h-6 w-6" />
            <span className="relative z-10 text-[10px] font-bold opacity-0 max-h-0 overflow-hidden">
              Menu
            </span>
          </button>
        </div>
